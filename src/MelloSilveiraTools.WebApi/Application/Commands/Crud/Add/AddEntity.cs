...
/// <summary>
/// Generic operation that inserts an entity through <see cref="IRepository"/> and returns an <see cref="AddResult"/>
...
public class AddEntity<TEntity>(IRepository repository)
    : CommandBase<AddEntityRequest<TEntity>, AddResult>
    where TEntity : EntityBase, new()
{
    /// <inheritdoc />
    protected override async Task<AddResult> ExecuteCommandAsync(AddEntityRequest<TEntity> request)
    {
        long id = await repository.InsertAsync(request.Entity).ConfigureAwait(false);
        return AddResult.CreateSuccessCreated(id);
    }
}
